...
| 22 | `src/components/bandi/BandoCard.astro` esiste ma nessuno lo importa | **Confermata, ed è peggio** | Tutti e 5 i file di `src/components/bandi/` erano codice morto. `BandoCard.astro` divergeva dal markup vivo: palette `slate-*`, `<a>` solo sul titolo, **manca la classe hook `bando-item`**, badge da `computeScadenzaStato` invece di `effectiveStatoBando`. **Aggiornamento (23/09/2026)**: la cartella è stata rimossa con l'intervento «fonti ufficiali e bandi attivi»; il markup vivo delle card sta in `src/components/liste/CardBando.astro` |
